...
class RootPage extends StatefulWidget {
  ...
  RootPage(this.type, {Key key, this.tabIndex = 0}) : super(key: key);
...
class _RootPageState extends State<RootPage> {
  // 하단 네비게이션 화면 키 리스트
  final List<GlobalKey<NavigatorState>> _keyList = [
    tabNavKey1,
    tabNavKey2,
    tabNavKey3,
    tabNavKey4,
  ...
  void initState() {
    ...
    _tabBarController.addListener(() {
      // 하단 네비게이션 탭
      if (_keyList[_tabBarController.index].currentState != null &&
          _keyList[_tabBarController.index].currentState.canPop()) {
        _keyList[_tabBarController.index]
            .currentState
            .popUntil((route) => route.isFirst); // root 페이지로 이동
      }
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    // 하단 네비게이션 화면 리스트
    List _screenList = [
      widget.type == CommonUtils.TYPE_MEMBER
          ? PremiumMemberPage()
          : PremiumTrainerPage(), // 프리미엄 관리
      Container(), // 게시판
    ...
    return WillPopScope(
      onWillPop: () async {
        if (_tabBarController.index == 2) {
          return !await _keyList[_tabBarController.index]
              .currentState
              .maybePop();
        } else {
          if (!await _keyList[_tabBarController.index]
              .currentState
              .maybePop()) {
        ...
        tabBar: CupertinoTabBar(
          items: [
            _buildBottomNavigationBarItem(
                'assets/ic_unselected_nav_1.png', '프리미엄 관리'),
            _buildBottomNavigationBarItem(
                'assets/ic_unselected_nav_2.png', '게시판'),
            _buildBottomNavigationBarItem(
                'assets/ic_unselected_nav_3.png', '트레이닝'),
            _buildBottomNavigationBarItem(
                'assets/ic_unselected_nav_4.png', '쇼핑'),
            _buildBottomNavigationBarItem('assets/img_empty_profile.png',
                '마이페이지'), // fixme :: 이미지를 다운 받을수 없어서 임시용
          ],
        ),
